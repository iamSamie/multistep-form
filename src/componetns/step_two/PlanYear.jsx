...
function PlanYear() {
    const { planChange, setPlanChange, setStepTwoCompleted, setPrice } = useContext(Context)
    const change = (event) => {
        setPlanChange(event.target.value)
        setStepTwoCompleted(true)
        if (event.target.value === 'Arcade') setPrice(9)
        if (event.target.value === 'Advanced') setPrice(12)
        if (event.target.value === 'Pro') setPrice(15)
    }

    return (
        <div className="plan">
            <label className={planChange === 'Arcade' ? 'arcade plan-checked' : 'arcade'}>
                <svg
                    className="step-two-svg"
                ...
                <div className="description-selected">
                    ...
                    <p className="month-free">2 month free</p>
                </div>
                <input
                    type="radio"
                    name="Arcade"
                    value="Arcade"
                    checked={planChange === 'Arcade' ? true : false}
                    onChange={change}
                />
            </label>
            <label className={planChange === 'Advanced' ? 'advanced plan-checked' : 'advanced'}>
                <svg
                    className="step-two-svg"
                    xmlns="http://www.w3.org/2000/svg"
                    width="40"
                ...
                <input
                    ...
                    name="Advanced"
                    value="Advanced"
                    checked={planChange === 'Advanced' ? true : false}
                    onChange={change}
                />
            </label>
            <label className={planChange === 'Pro' ? 'pro plan-checked' : 'pro'}>
                <svg
                    className="step-two-svg"
                    xmlns="http://www.w3.org/2000/svg"
                    width="40"
                ...
                <input
                    ...
                    name="Pro"
                    value="Pro"
                    checked={planChange === 'Pro' ? true : false}
                    onChange={change}
                />
            </label>
        </div>
